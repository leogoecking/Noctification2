...
type TaskQueueFilter = "all" | "attention" | "due_today" | "overdue" | "blocked" | "stale" | "unassigned";
type MetricsWindow = "7d" | "30d";

interface MetricsTaskItem {
  id: number;
  title: string;
  description: string;
  status: TaskStatus;
...
interface TaskMetricsProductivity {
  windowDays: number;
...
interface TaskMetricsCapacityByAssignee {
  assigneeKey: string;
...
interface TaskMetricsCapacityByDepartment {
...
const buildTaskSlaKind = (task: MetricsTaskItem, now = new Date()): TaskSlaKind => {
...
const matchesTaskQueueFilter = (
  ...
  if (queueFilter === "stale") {
    return sla === "stale" || (task.status === "blocked" && now.getTime() - new Date(task.updatedAt).getTime() >= 24 * 60 * 60 * 1000);
  }
...
const buildCapacityByAssignee = (tasks: MetricsTaskItem[], now = new Date()): TaskMetricsCapacityByAssignee[] => {
...
const buildCapacityByDepartment = (
  tasks: MetricsTaskItem[],
...
export const buildTaskMetricsSummary = (
  ...
  return {
    productivity: buildProductivity(tasks, params.metricsWindow, now),
    capacityByAssignee: buildCapacityByAssignee(tasks, now),
    capacityByDepartment: buildCapacityByDepartment(tasks, users, now)
  };
};
