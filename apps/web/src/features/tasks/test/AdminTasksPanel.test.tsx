...
describe("AdminTasksPanel", () => {
  ...
  it("fecha o detalhe admin quando a tarefa eh excluida pelo detalhe", async () => {
    mockedApi.adminTasks
      .mockResolvedValueOnce({
    ...
    mockedApi.cancelAdminTask.mockResolvedValue({
      task: { ...buildAdminTask({ id: 12, title: "Tarefa 12" }), status: "cancelled" }
    });
    ...
    await waitFor(() => expect(mockedApi.cancelAdminTask).toHaveBeenCalledWith(12));
    await waitFor(() =>
  ...
  it("permite editar a tarefa pelo detalhe administrativo", async () => {
    mockedApi.adminTasks
      .mockResolvedValueOnce({
        tasks: [
          {
    ...
    mockedApi.updateAdminTask.mockResolvedValue({
      task: buildAdminTask({ id: 11, title: "Tarefa 11 ajustada" })
    });
    ...
    fireEvent.change(screen.getByPlaceholderText("Titulo da tarefa"), {
      target: { value: "Tarefa 11 ajustada" }
    });
    fireEvent.click(screen.getByRole("button", { name: "Salvar tarefa" }));

    await waitFor(() =>
      expect(mockedApi.updateAdminTask).toHaveBeenCalledWith(
        11,
        expect.objectContaining({ title: "Tarefa 11 ajustada" })
      )
    );
  });
  ...
  it("conclui a tarefa ao arrastar para a coluna concluida no board administrativo", async () => {
    mockedApi.adminTasks
      .mockResolvedValueOnce({
        tasks: [
          buildAdminTask({
            id: 112,
            title: "Drag para concluir",
            status: "in_progress"
          })
        ],
        pagination: { page: 1, limit: 100, total: 1, totalPages: 1 }
    ...
    mockedApi.adminTask.mockResolvedValue({
      task: buildAdminTask({ id: 112, title: "Drag para concluir" }),
      timeline: []
    });
    ...
    await waitFor(() => expect(mockedApi.completeAdminTask).toHaveBeenCalledWith(112));
  });
  ...
  it("exibe saude da automacao no modal de indicadores", async () => {
    mockedApi.adminTasks.mockResolvedValue({
      tasks: [
        buildAdminTask({
          id: 60,
  ...
  it("envia busca textual de tarefas do admin", async () => {
    ...
    await waitFor(() => expect(mockedApi.adminTasks).toHaveBeenCalledTimes(1));
    fireEvent.change(screen.getByPlaceholderText("titulo, descricao, responsavel ou status"), {
