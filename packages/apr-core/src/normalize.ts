import {
  APR_DB_KEY,
  APR_SNAPSHOT_VERSION,
  DEFAULT_EMPLOYEES,
  EMPLOYEE_TOKEN_IGNORES
} from "./constants.js";
...
export const unwrapSpreadsheetFormulaText = (value: unknown): string => {
...
export const normalizeHeader = (header: unknown): string => normalizeText(header).toLowerCase();

export const normalizeSubjectPattern = (value: unknown): string => {
...
export const employeeNameTokens = (value: unknown): string[] =>
  employeeNameKey(value)
    .split(" ")
    .filter((token) => token && !EMPLOYEE_TOKEN_IGNORES.has(token));

export const canonicalEmployeeMatch = (value: unknown): string => {
...
export const normalizeEmployeeName = (value: unknown): string => canonicalEmployeeMatch(value);
...
const excelSerialDateToIso = (value: number): string => {
  if (!Number.isFinite(value) || value <= 0) {
    return "";
  }

  const baseUtc = Date.UTC(1899, 11, 30);
  const date = new Date(baseUtc + Math.trunc(value) * 86_400_000);
...
export const formatDateBr = (value: unknown, emptyValue = "-"): string => {
...
export const monthFromIsoDate = (dateStr: unknown): string =>
  isValidIsoDate(dateStr) ? String(dateStr).slice(0, 7) : "";
...
export const sortAprRows = (rows: AprEntry[]): AprEntry[] =>
  rows.sort((a, b) => String(a.ID).localeCompare(String(b.ID), "pt-BR", { numeric: true }));

export const normalizeEntryShape = (entry: unknown): AprEntry | null => {
  if (!entry || typeof entry !== "object") {
    return null;
  }

  const candidate = entry as Partial<AprEntry>;
...
export const normalizeMonthPackShape = (pack: unknown): AprMonthPack => {
  const candidate = pack as Partial<AprMonthPack> | null | undefined;
  const manual = Array.isArray(candidate?.manual)
    ? candidate.manual.map(normalizeEntryShape).filter((item): item is AprEntry => item !== null)
    : [];
  const system = Array.isArray(candidate?.system)
    ? candidate.system.map(normalizeEntryShape).filter((item): item is AprEntry => item !== null)
    : [];
  const imports =
    candidate && typeof candidate.imports === "object" && !Array.isArray(candidate.imports)
      ? candidate.imports
      : {};
...
export const initialDb = (now = new Date().toISOString()): AprDb => ({
  months: {},
  meta: {
    createdAt: now,
    updatedAt: now
...
export const normalizeDbShape = (db: unknown, now = new Date().toISOString()): AprDb => {
  const safe = initialDb(now);
  if (!db || typeof db !== "object") {
    return safe;
  }

  const candidate = db as Partial<AprDb>;
  const rawMonths =
    candidate.months && typeof candidate.months === "object" && !Array.isArray(candidate.months)
      ? candidate.months
      : {};
  ...
  const rawMeta: Partial<AprDb["meta"]> =
...
export const parseDbString = (raw: unknown): AprDb | null => {
...
export const cloneDb = (db: unknown): AprDb => {
  try {
...
export const integrityChecksum = (at: string, reason: string, data: string): string => {
  let hash = 2166136261;
...
export const normalizeSnapshotEnvelope = (item: unknown): AprSnapshotEnvelope | null => {
  if (!item || typeof item !== "object") {
    return null;
  }

  const candidate = item as Partial<AprSnapshotEnvelope>;
