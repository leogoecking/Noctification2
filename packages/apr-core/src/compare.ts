...
export const buildRowMap = (rows: AprEntry[]): Map<string, AprEntry> => {
  const result = new Map<string, AprEntry>();
...
export const compareBases = (
  systemRows: AprEntry[],
  manualRows: AprEntry[]
): { summary: AprAuditSummary; details: AprAuditDetail[] } => {
...
export const compareMonthToPrevious = (
  currentRows: AprEntry[],
  previousRows: AprEntry[]
): { summary: AprHistorySummary; details: AprHistoryDetail[] } => {
  const currentMap = buildRowMap(currentRows);
  const previousMap = buildRowMap(previousRows);
  const ids = [...currentMap.keys()].sort((a, b) =>
    String(a).localeCompare(String(b), "pt-BR", { numeric: true })
  );

  const summary: AprHistorySummary = {
