...
const localNameOf = (node) => (node?.localName ?? node?.nodeName ?? "")
    .split(":")
    .pop()
    ?.trim() ?? "";
const isElementNode = (node) => Boolean(node) && node?.nodeType === node?.ELEMENT_NODE;
const getDirectChildElement = (element, tagName) => {
...
const setDirectChildText = (element, tagName, value) => {
    const child = getDirectChildElement(element, tagName);
    if (child) {
...
const hasDescendantTag = (element, tagName) => collectDescendants(element).some((child) => localNameOf(child) === tagName);
...
const getPlacemarkElementsInDocumentOrder = (doc) => {
    const items = [];
...
const buildName = (prefix, value, minPadding) => {
...
const decidePlacemark = (placemark, mode, ignoredNames) => {
    ...
    const hasExcludedGeometry = hasDescendantTag(placemark, "LineString") ||
    ...
    const hasExcludedKeyword = EXCLUDED_KEYWORDS.some((keyword) => combinedNormalized.includes(keyword));
...
export const standardizePosteKml = (xml, options) => {
